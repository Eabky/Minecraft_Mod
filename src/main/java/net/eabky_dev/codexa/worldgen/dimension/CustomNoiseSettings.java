...
import java.util.stream.Stream;
    ...
    /*
        Credit goes to https://github.com/LuciferC-137/MC-Mod_ABYSS/tree/main
    */
...
public class CustomNoiseSettings
{
    ...
    public static final ResourceKey<DensityFunction> CONTINENTS = createKey("overworld_large_biomes/continents");
    public static final ResourceKey<DensityFunction> EROSION = createKey("overworld_large_biomes/erosion");
    public static final ResourceKey<DensityFunction> RIDGES = createKey("end/sloped_cheese");
    public static final ResourceKey<DensityFunction> FACTOR = createKey("overworld/factor");
    public static final ResourceKey<DensityFunction> DEPTH = createKey("overworld_large_biomes/depth");
    ...
    public static final Codec<NoiseGeneratorSettings> DIRECT_CODEC = RecordCodecBuilder.create((p_64475_) ->
    {
        return p_64475_.group(NoiseSettings.CODEC.fieldOf("noise").forGetter(NoiseGeneratorSettings::noiseSettings),
                        ...
                        Codec.BOOL.fieldOf("legacy_random_source").forGetter(NoiseGeneratorSettings::useLegacyRandomSource)).apply(p_64475_, NoiseGeneratorSettings::new);
    });
    ...
    {
        ctx.register(MIDNIGHT_SEA_NOISE, createAbyssNoiseSettings(ctx, false, false));
    }

    private static NoiseGeneratorSettings createAbyssNoiseSettings(BootstapContext<?> p_256460_, boolean amplified, boolean large)
    {
        return new NoiseGeneratorSettings(
                NoiseSettings.create(-64, 384, 4, 4),
                CodexaModBlocks.MIDNIGHT_STONE.get().defaultBlockState(),
                Blocks.WATER.defaultBlockState(),
                overworld(p_256460_.lookup(Registries.DENSITY_FUNCTION), p_256460_.lookup(Registries.NOISE), large, amplified),
                ModSurfaceRules.makeRules(),
                (new OverworldBiomeBuilder()).spawnTarget(), 91, false, false, false, false);
    }
    ...
    private static DensityFunction underground(HolderGetter<DensityFunction> densFuncHold, HolderGetter<NormalNoise.NoiseParameters> noiseParam, DensityFunction densFunc)
    {
        ...
        DensityFunction densityfunction3 = DensityFunctions.mul(DensityFunctions.constant(3.0D), densityfunction2.square());DensityFunction densityfunction4 = DensityFunctions.noise(noiseParam.getOrThrow(Noises.CAVE_CHEESE), 0.6666666666666666D);
        DensityFunction densityfunction5 = DensityFunctions.add(DensityFunctions.add(DensityFunctions.constant(0.27D), densityfunction4).clamp(-1.0D, 1.0D), DensityFunctions.add(DensityFunctions.constant(1.5D), DensityFunctions.mul(DensityFunctions.constant(-0.64D), densFunc)).clamp(-0.178D, 0.5D));
